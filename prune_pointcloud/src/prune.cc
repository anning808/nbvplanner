...
void PointcloudPruning::Prune::pointcloud(const sensor_msgs::PointCloud2::ConstPtr& pointcloudIn)
{
  ...
  for (typename std::vector<std::string>::iterator it = vehicle_tf_frames_.begin();
    ...
    try {
      tf_listener_.waitForTransform(pointcloudIn->header.frame_id, *it, ros::Time(0), ros::Duration(3.0));
      tf_listener_.lookupTransform(pointcloudIn->header.frame_id, *it, ros::Time(0),
                                   tf_transform);
