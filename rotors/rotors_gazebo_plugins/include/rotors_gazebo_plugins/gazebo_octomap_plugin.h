...
class OctomapFromGazeboWorld : public WorldPlugin {
 ...
 protected:
  ...
  bool CheckIfInterest(const ignition::math::Vector3<double>& central_point,
                       gazebo::physics::RayShapePtr ray,
                       const double leaf_size);
  void FloodFill(const ignition::math::Vector3<double>& seed_point,
                 const ignition::math::Vector3<double>& bounding_box_origin,
                 const ignition::math::Vector3<double>& bounding_box_lengths,
                 const double leaf_size);
