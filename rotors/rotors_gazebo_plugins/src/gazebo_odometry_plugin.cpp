...
void GazeboOdometryPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  ignition::math::Vector3<double> noise_normal_position;
  ignition::math::Vector3<double> noise_normal_quaternion;
  ignition::math::Vector3<double> noise_normal_linear_velocity;
  ignition::math::Vector3<double> noise_normal_angular_velocity;
  ignition::math::Vector3<double> noise_uniform_position;
  ignition::math::Vector3<double> noise_uniform_quaternion;
  ignition::math::Vector3<double> noise_uniform_linear_velocity;
  ignition::math::Vector3<double> noise_uniform_angular_velocity;
  const ignition::math::Vector3<double> zeros3(0.0, 0.0, 0.0);
  ...
  getSdfParam<ignition::math::Vector3<double>>(_sdf, "noiseNormalPosition", noise_normal_position, zeros3);
  getSdfParam<ignition::math::Vector3<double>>(_sdf, "noiseNormalQuaternion", noise_normal_quaternion, zeros3);
  getSdfParam<ignition::math::Vector3<double>>(_sdf, "noiseNormalLinearVelocity", noise_normal_linear_velocity, zeros3);
  getSdfParam<ignition::math::Vector3<double>>(_sdf, "noiseNormalAngularVelocity", noise_normal_angular_velocity, zeros3);
  getSdfParam<ignition::math::Vector3<double>>(_sdf, "noiseUniformPosition", noise_uniform_position, zeros3);
  getSdfParam<ignition::math::Vector3<double>>(_sdf, "noiseUniformQuaternion", noise_uniform_quaternion, zeros3);
  getSdfParam<ignition::math::Vector3<double>>(_sdf, "noiseUniformLinearVelocity", noise_uniform_linear_velocity, zeros3);
  getSdfParam<ignition::math::Vector3<double>>(_sdf, "noiseUniformAngularVelocity", noise_uniform_angular_velocity, zeros3);
...
void GazeboOdometryPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  ignition::math::Vector3<double> gazebo_linear_velocity = C_linear_velocity_W_C;
  ignition::math::Vector3<double> gazebo_angular_velocity = C_angular_velocity_W_C;
  ...
  if (parent_frame_id_ != kDefaultParentFrameId) {
    ...
    ignition::math::Vector3<double> P_linear_velocity_W_P = parent_link_->RelativeLinearVel();
    ignition::math::Vector3<double> P_angular_velocity_W_P = parent_link_->RelativeAngularVel();
      ignition::math::Pose3d  C_pose_P_C_ = W_pose_W_C - W_pose_W_P;
    ignition::math::Vector3<double> C_linear_velocity_P_C;
  ...
  if (covariance_image_.data != NULL) {
    ...
    int x = static_cast<int>(std::floor(gazebo_pose.Pos()[0] / covariance_image_scale_)) + width / 2;
    int y = static_cast<int>(std::floor(gazebo_pose.Pos()[1] / covariance_image_scale_)) + height / 2;
  ...
  if (gazebo_sequence_ % measurement_divisor_ == 0) {
    ...
    odometry.twist.twist.linear.x = gazebo_linear_velocity[0];
    odometry.twist.twist.linear.y = gazebo_linear_velocity[1];
    odometry.twist.twist.linear.z = gazebo_linear_velocity[2];
  ...
  if (gazebo_sequence_ == odometry_queue_.front().first) {
    ...
    //    std::cout << "published odometry with timestamp " << odometry->header.stamp << "at time t" << world_->SimTime().Double()
