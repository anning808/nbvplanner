...
void GazeboMavlinkInterface::OnUpdate(const common::UpdateInfo& /*_info*/) {
  ...
  ignition::math::Vector3<double> pos_W_I = T_W_I.Pos();  // Use the models' world position for GPS and pressure alt.

  ignition::math::Vector3<double> velocity_current_W = model_->WorldLinearVel();  // Use the models' world position for GPS velocity.

  ignition::math::Vector3<double> velocity_current_W_xy = velocity_current_W;
  ...
  if(current_time - last_gps_time_ > gps_update){  // 5Hz
    ...
    hil_gps_msg_.vel = velocity_current_W_xy.Length() * 100;
...
void GazeboMavlinkInterface::ImuCallback(const sensor_msgs::ImuConstPtr& imu_message) {
  ...
  ignition::math::Quaterniond C_W_I;
  ...
  ignition::math::Vector3<double> mag_I = C_W_I.RotateVectorReverse(mag_W_); // TODO: Add noise based on bais and variance like for imu and gyro
